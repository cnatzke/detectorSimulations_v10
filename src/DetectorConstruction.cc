...
DetectorConstruction::DetectorConstruction() : fSolidWorld(nullptr),
											   fLogicWorld(nullptr),
											   fPhysiWorld(nullptr)
{
...
void DetectorConstruction::LayeredTargetAdd(G4String Material, G4double Areal)
{
	if (!fApparatusLayeredTarget)
		fApparatusLayeredTarget = new ApparatusLayeredTarget(fDetEffPosition.z());
...
void DetectorConstruction::AddApparatusPEEKSourceHolder(G4int position)
{
	// Place source holder in position <position>
	ApparatusPEEKSourceHolder *pApparatusPEEKSourceHolder = new ApparatusPEEKSourceHolder();
	pApparatusPEEKSourceHolder->Build();
	pApparatusPEEKSourceHolder->Place(fLogicWorld, position);
}
...
void DetectorConstruction::AddDetectionSystemDescantAuxPorts(G4ThreeVector input)
{
	...
	if (leadShield)
		G4cout << "Building DESCANT detectors WITH lead shielding" << G4endl;
	if (!leadShield)
		G4cout << "Building DESCANT detectors WITHOUT lead shielding" << G4endl;
