	...
	: fAction(Gun)
{
	...
	fSourceRadiusCmd = new G4UIcmdWithADoubleAndUnit("/DetSys/gun/sourceRadius", this); // source spot sigma
	fSourceRadiusCmd->SetGuidance("Set radius of source");
	fSourceRadiusCmd->SetUnitCategory("Length");
	fSourceRadiusCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
...
void PrimaryGeneratorMessenger::SetNewValue(G4UIcommand *command, G4String newValue)
{
	...
	if (command == fSourceRadiusCmd)
	{
		fAction->SetSourceRadius(fSourceRadiusCmd->GetNewDoubleValue(newValue));
		G4cout << "Source radius set to: " << fSourceRadiusCmd->GetNewDoubleValue(newValue) << G4endl;
		return;
	}
